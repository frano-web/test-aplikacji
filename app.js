...
async function signupSubmit(e){
  ...
  const { error }=await supabase.auth.signUp({email:f.get('email'),password:f.get('password'),options:{data:{full_name:String(f.get('full_name')).trim(),invite_code:code}}});
...
async function loadBase(){
  const [trips,events,ann,dir,chats]=await Promise.all([
    supabase.from('trips').select('*').order('starts_at',{ascending:true}),
    supabase.from('calendar_events').select('*').order('starts_at',{ascending:true}).limit(120),
    supabase.from('announcements').select('*').order('created_at',{ascending:false}).limit(50),
    supabase.from('directory').select('*').eq('active',true).order('full_name'),
  ...
  if(role()==='athlete'){
    const {data:mine,error}=await supabase.from('trip_members').select('trip_id').eq('user_id',state.session.user.id);
  ...
  state.chatLabels={};
  await Promise.all(state.chatGroups.filter(g=>g.kind==='direct').map(async g=>{const {data}=await supabase.from('chat_members').select('user_id').eq('group_id',g.id);const other=(data||[]).find(x=>x.user_id!==state.session.user.id);state.chatLabels[g.id]=other?personName(other.user_id):'Rozmowa prywatna';}));
...
async function loadUnread(){
  ...
  const {data,error}=await supabase.from('announcement_reads').select('announcement_id').eq('user_id',state.session.user.id).in('announcement_id',ids);
...
function shell(){
  ...
  document.querySelector('#logout-btn').onclick=()=>supabase.auth.signOut();
...
async function getTripStats(tripId){
  ...
  const [m,t,c]=await Promise.all([
    supabase.from('trip_members').select('status,bike_status,bike_count').eq('trip_id',tripId),
    supabase.from('tasks').select('done').eq('trip_id',tripId),
    supabase.from('trip_vehicles').select('id').eq('trip_id',tripId)
  ]);
...
async function renderTripMembers(trip,el){
  const {data,error}=await supabase.from('trip_members').select('*, profiles:user_id(full_name), trip_vehicles(id,vehicles(name),driver_id), pickup_stops(label,pickup_time)').eq('trip_id',trip.id).order('updated_at');
...
async function renderShopping(trip,el){
  const {data,error}=await supabase.from('shopping_items').select('*').eq('trip_id',trip.id).order('created_at'); if(error){el.innerHTML=esc(error.message);return;}
  ...
  document.querySelector('#shop-toggle')?.addEventListener('click',async()=>{const {error}=await supabase.from('trips').update({shopping_enabled:!trip.shopping_enabled}).eq('id',trip.id);if(error)return fail(error);trip.shopping_enabled=!trip.shopping_enabled;renderTrips();});
  document.querySelector('#shop-form')?.addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(e.currentTarget);const {error}=await supabase.from('shopping_items').insert({trip_id:trip.id,item:String(f.get('item')).trim(),created_by:state.session.user.id});if(error)return fail(error);renderTripTab(trip);});
  document.querySelectorAll('.shop-done').forEach(c=>c.onchange=async()=>{const {error}=await supabase.from('shopping_items').update({done:c.checked}).eq('id',c.dataset.id);if(error){c.checked=!c.checked;fail(error);}});
}

async function renderChecklist(trip,el){
  const {data,error}=await supabase.from('tasks').select('*,person:assigned_to(full_name)').eq('trip_id',trip.id).order('created_at');if(error){el.innerHTML=esc(error.message);return;}
  el.innerHTML=`<div class="toolbar" style="margin-top:12px"><div class="muted">${data.filter(x=>!x.done).length} otwartych</div>${staff()?'<button class="primary" id="task-add" type="button">+ Zadanie</button>':''}</div><div class="card" style="margin-top:10px">${data.map(x=>`<div class="row"><label class="check"><input class="task-check" data-id="${x.id}" type="checkbox" ${x.done?'checked':''}><span>${esc(x.title)}</span></label><span class="pill">${esc(x.person?.full_name||ROLES[x.assigned_role]||'Bez przypisania')}</span></div>`).join('')||'<div class="empty">Brak zadań.</div>'}</div>`; document.querySelector('#task-add')?.addEventListener('click',()=>taskModal(trip.id)); document.querySelectorAll('.task-check').forEach(c=>c.onchange=async()=>{const {error}=staff()?await supabase.from('tasks').update({done:c.checked}).eq('id',c.dataset.id):await supabase.rpc('set_my_task_done',{p_task_id:c.dataset.id,p_done:c.checked});if(error){c.checked=!c.checked;fail(error);}});
}
...
async function renderMyTrip(){
  ...
  const {data:member,error}=await supabase.from('trip_members').select('*,trip_vehicles(id,driver_id,vehicles(name)),pickup_stops(id,label,pickup_time)').eq('trip_id',trip.id).eq('user_id',state.session.user.id).maybeSingle(); if(error)return fail(error);
  ...
  const {data:stops}=member.trip_vehicle_id?await supabase.from('pickup_stops').select('*').eq('trip_vehicle_id',member.trip_vehicle_id).order('sort_order'):{data:[]};
...
async function renderTeam(){
  if(!staff()) return navigate('dashboard'); setTitle('ZESPÓŁ','Użytkownicy i role'); const {data,error}=await supabase.from('profiles').select('id,full_name,role,phone,active').order('full_name'); if(error)return fail(error);
...
async function loadMessages(groupId){ const {data,error}=await supabase.from('messages').select('id,body,created_at,sender_id').eq('group_id',groupId).order('created_at').limit(150);if(error)return fail(error); const el=document.querySelector('#messages');if(!el)return; el.innerHTML=(data||[]).map(m=>`<div class="bubble ${m.sender_id===state.session.user.id?'me':''}"><small>${esc(personName(m.sender_id))} · ${new Date(m.created_at).toLocaleTimeString('pl-PL',{hour:'2-digit',minute:'2-digit'})}</small>${esc(m.body)}</div>`).join('')||'<div class="empty">Napisz pierwszą wiadomość.</div>';el.scrollTop=el.scrollHeight; }
async function sendMessage(e){e.preventDefault();const f=new FormData(e.currentTarget),body=String(f.get('body')).trim();if(!body)return;const {error}=await supabase.from('messages').insert({group_id:state.activeChatId,sender_id:state.session.user.id,body});if(error)return fail(error);e.currentTarget.reset();}
...
async function markAnnouncementRead(id,close=true){
  const {error}=await supabase.rpc('mark_announcement_read',{p_announcement_id:id});
